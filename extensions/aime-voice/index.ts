...
import { ContactMemoryManager } from '../contact-memory/contact-memory/index.js';
...
const aimeVoicePlugin: OpenClawPluginDefinition = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Register outbound calling tool for agent
    api.registerTool({
      name: 'make_outbound_call',
      description: 'Make an outbound phone call to a contact using LiveKit voice AI. Use this to follow up with leads, confirm appointments, or conduct sales calls.',
      parameters: {
        type: 'object',
        properties: {
          phoneNumber: {
            type: 'string',
            description: 'Phone number to call in E.164 format (e.g., +18508428707)'
          },
          contactId: {
            type: 'string',
            description: 'GoHighLevel contact ID (optional, used to load contact context)'
          },
          purpose: {
            type: 'string',
            description: 'Purpose of the call (e.g., "voice automation inquiry follow-up")'
          },
          script: {
            type: 'string',
            description: 'Call script or talking points (optional)'
          },
          reportTo: {
            type: 'string',
            description: 'Telegram user to send report to after call (e.g., "@line_logic")'
          }
        },
      ...
      execute: async (params) => {
        try {
          ...
          // Load contact context if contactId provided
          let contactContext = null;
          if (params.contactId) {
            try {
              contactContext = await bridge.getContactContext(params.contactId);
            } catch (error) {
              api.logger.warn('[AIME Voice] Could not load contact context:', error);
            }
          }

          // Initiate the call
          const callResult = await initiator.initiateCall({
